 ...
 */
export function generateVertexLabel(index) {
 ...
 */
export function edgeExists(edges, uId, vId) {
    return edges.some(e => (e.source === uId && e.target === vId) || (e.source === vId && e.target === uId));
}
/**
 * Tìm ID của cạnh nối giữa u và v
 */
export function findEdgeId(edges, uId, vId) {
    const edge = edges.find(e => (e.source === uId && e.target === vId) || (e.source === vId && e.target === uId));
    return edge?.id;
}
/**
 ...
 */
export function calculateDegrees(graph) {
 ...
 */
export function findConnectedComponents(graph, nonZeroOnly = false) {
    const adj = buildAdjacencyList(graph);
 ...
 */
export function applyCircularLayout(vertices, width = 600, height = 500, padding = 80) {
 ...
 * "1 2\n2 3"
 */
export function parseEdgeList(input, width = 600, height = 500) {
    const lines = input
        .split(/[\n;]/)
        .map(l => l.trim())
        .filter(l => l.length > 0);
    ...
    const vertices = labelList.map((label, i) => {
 ...
 */
export function exportToEdgeList(graph) {
