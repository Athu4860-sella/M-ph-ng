 ...
 */
export function analyzeEuler(graph) {
    ...
    const evenDegreeVertices = [];
    const zeroDegreeVertices = [];
    ...
    if (!isConnectedNonZero) {
        const componentStr = nonZeroComponents
            .map((c, idx) => `Nhóm ${idx + 1} (${c.map(id => vertexMap.get(id)?.label).join(', ')})`)
            .join('; ');
        return {
            type: 'none',
            isAllConnectedNonZero: false,
 ...
 */
export function runHierholzerSimulation(graph) {
    ...
    const stack = [startId];
    const circuit = []; // Thu thập các đỉnh khi pop khỏi stack
    const traversedEdgesList = [];
    const visitedEdgeSet = new Set();
    const visitedVertexSet = new Set([startId]);
    let stepCounter = 0;
    // Bước 0: Khởi tạo
    steps.push({
