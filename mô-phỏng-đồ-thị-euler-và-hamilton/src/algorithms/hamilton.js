 ...
 */
export function analyzeHamilton(graph) {
    ...
    const theoreticalNotes = [];
    if (diracSatisfied) {
        theoreticalNotes.push(`Thỏa mãn Định lý Dirac: Mọi đỉnh đều có bậc ≥ ${diracThreshold} (n/2 = ${n}/2). Đảm bảo tồn tại Chu trình Hamilton.`);
    }
    else {
        theoreticalNotes.push(`Không thỏa Định lý Dirac (đòi hỏi mọi đỉnh bậc ≥ ${diracThreshold}, bậc nhỏ nhất hiện tại là ${minDegree}).`);
    }
    if (oreSatisfied) {
        theoreticalNotes.push(`Thỏa mãn Định lý Ore: Mọi cặp đỉnh không kề nhau đều có tổng bậc ≥ ${n}. Đảm bảo tồn tại Chu trình Hamilton.`);
    }
    else {
        theoreticalNotes.push(`Không thỏa Định lý Ore (tồn tại cặp đỉnh không kề nhau có tổng bậc < ${n}).`);
    }
    if (!isConnected) {
        theoreticalNotes.push(`Đồ thị không liên thông (có ${components.length} thành phần rời rạc). Không thể có chu trình Hamilton đi qua tất cả các đỉnh.`);
    }
    if (hasDegreeLessThanTwo) {
        const lowDegreeVertices = degreeInfos
            ...
            .join(', ');
        theoreticalNotes.push(`Điều kiện cần bị vi phạm: Có đỉnh bậc nhỏ hơn 2 (${lowDegreeVertices}). Mọi đỉnh trong chu trình Hamilton đều phải có ít nhất 2 cạnh liên thuộc.`);
    }
    return {
        analysis: {
 ...
 */
export function runHamiltonSimulation(graph) {
    ...
    const path = [startVertex.id];
    const inPath = new Set([startVertex.id]);
    ...
    function backtrack(currId) {
        ...
        for (const nextId of neighbors) {
            ...
            if (!inPath.has(nextId)) {
                ...
                if (edgeIdx !== -1)
                    traversedEdges.splice(edgeIdx, 1);
                const vEdgeIdx = visitedEdgeIds.lastIndexOf(edgeId);
                if (vEdgeIdx !== -1)
                    visitedEdgeIds.splice(vEdgeIdx, 1);
                steps.push({
                    stepIndex: stepCounter++,
                    algorithm: 'hamilton',
                    status: 'backtracking',
...
//# sourceMappingURL=hamilton.js.map